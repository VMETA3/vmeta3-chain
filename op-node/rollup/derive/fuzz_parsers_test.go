...
var (
	...
	portalContract, _      = bindings.NewVmeta3Portal(addr, nil)
...
func FuzzUnmarshallLogEvent(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, _to, _mint, _value, data []byte, l2GasLimit uint64, isCreation bool) {
		...
		state.SetCode(addr, common.FromHex(bindings.Vmeta3PortalDeployedBin))
		...
		reconstructed := &bindings.Vmeta3PortalTransactionDeposited{
			From:       dep.From,
			Value:      dep.Value,
			GasLimit:   dep.Gas,
		...
		inputArgs := &bindings.Vmeta3PortalTransactionDeposited{
			From:       from,
			To:         to,
			Mint:       mint,
			Value:      value,
