...
import (
	...
	"github.com/VMETA3/vmeta3-chain/op-node/rollup"
...
// The discovery ENRs are just key-value lists, and we filter them by records tagged with the "vmeta3" key,
// and then check the chain ID and version.
type Vmeta3ENRData struct {
	chainID uint64
	version uint64
}

func (o *Vmeta3ENRData) ENRKey() string {
	return "vmeta3"
}

func (o *Vmeta3ENRData) EncodeRLP(w io.Writer) error {
...
func (o *Vmeta3ENRData) DecodeRLP(s *rlp.Stream) error {
	b, err := s.Bytes()
	if err != nil {
	...
	// Future "vmeta3" entries may contain additional data, and will be tagged with a newer version etc.
	r := bytes.NewReader(b)
...
func (n *NodeP2P) DiscoveryProcess(ctx context.Context, log log.Logger, cfg *rollup.Config, connectGoal uint) {
	...
	for {
		...
		case found := <-randomNodesCh:
			...
			n.ConnectionManager().TagPeer(info.ID, fmt.Sprintf("vmeta3-%d-%d", dat.chainID, dat.version), 42)
