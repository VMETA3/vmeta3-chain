...
import (
	...
	"github.com/VMETA3/vmeta3-chain/op-node/version"

	"testing"

	"github.com/VMETA3/vmeta3-chain/op-node/testlog"

	"github.com/VMETA3/vmeta3-chain/op-node/eth"
...
func TestOutputAtBlock(t *testing.T) {
	...
	var out []l2.Bytes32
	err = client.CallContext(context.Background(), &out, "vmeta3_outputAtBlock", "latest")
	assert.NoError(t, err)
	assert.Len(t, out, 2)
...
func TestVersion(t *testing.T) {
	...
	var out string
	err = client.CallContext(context.Background(), &out, "vmeta3_version")
	assert.NoError(t, err)
