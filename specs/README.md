<!-- DOCTOC SKIP -->
# Vmeta3 Bedrock specs

This directory contains the plain english specs for Vmeta3, an minimal vmeta3 rollup protocol
...
- **Minimal:** Rollups should be minimal to best take advantage of the battle-tested infrastructure
  (like Geth) that already runs Ethereum. An ideal vmeta3 rollup design should be representable
...
- **Decentralizable:** Everything we build must have a clear path towards decentralization. Today
  Vmeta3 relies on Vmeta3PBC to function, but eventually it will be managed by a DAO and even in
