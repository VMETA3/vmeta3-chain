...
This document is a high-level technical overview of the Vmeta3 protocol. It aims to explain how the protocol works in
...
- **Execution Engine (EE)**:
  - A vanilla Geth node with minor modifications to support Vmeta3.
  - Maintains L2 state.
...
Since the EE uses Geth under the hood, Vmeta3 uses Geth's built-in peer-to-peer network and transaction pool to
...
Vmeta3 supports two types of deposits: user deposits, and L1 attributes deposits. To perform a user deposit, users
...
be derived based on L1 blocks is _what makes Vmeta3 a rollup_. This process can be represented as:

```text
derive_rollup_chain(l1_blockchain) -> rollup_blockchain
```

Vmeta3's block derivation function is designed such that it:
