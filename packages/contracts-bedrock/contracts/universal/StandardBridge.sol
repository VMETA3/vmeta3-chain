...
import { Vmeta3MintableERC20 } from "./Vmeta3MintableERC20.sol";
...
abstract contract StandardBridge {
    ...
    function completeOutboundTransfer(
        ...
        if (_isVmeta3MintableERC20(_localToken)) {
            require(
                _isCorrectTokenPair(_localToken, _remoteToken),
                "Wrong remote token for Vmeta3 Mintable ERC20 local token"
            );

            Vmeta3MintableERC20(_localToken).mint(_to, _amount);
        } else {
    ...
    function _initiateBridgeERC20(
        ...
        if (_isVmeta3MintableERC20(_localToken)) {
            ...
            Vmeta3MintableERC20(_localToken).burn(msg.sender, _amount);
        } else {
    ...
    /**
     * Checks if a given address is an Vmeta3MintableERC20. Not perfect, but good enough.
     * Just the way we like it.
     *
     * @param _token Address of the token to check.
     * @return True if the token is an Vmeta3MintableERC20.
     */
    function _isVmeta3MintableERC20(address _token) internal view returns (bool) {
        // 0x1d1d8b63 is mint ^ burn ^ l1Token
        return ERC165Checker.supportsInterface(_token, 0x1d1d8b63);
    }

    /**
     * Checks if the "other token" is the correct pair token for the Vmeta3MintableERC20.
     *
     * @param _mintableToken Vmeta3MintableERC20 to check against.
     * @param _otherToken Pair token to check.
     * @return True if the other token is the correct pair token for the Vmeta3MintableERC20.
     */
    function _isCorrectTokenPair(address _mintableToken, address _otherToken)
        ...
        returns (bool)
    {
        return _otherToken == Vmeta3MintableERC20(_mintableToken).l1Token();
    }
}
