...
#    timestamps recovered in step 4 as well as the address of the Vmeta3Portal
...
if [ ! -f $CONTRACTS_BEDROCK/deployments/devnetL1/Vmeta3Portal.json ]; then
...
VMETA3_MINTABLE_TOKEN_FACTORY_BYTECODE=$(get_deployed_bytecode universal/Vmeta3MintableTokenFactory.sol/Vmeta3MintableTokenFactory.json)
...
DEPOSIT_CONTRACT_ADDRESS=$(jq -r .address < $CONTRACTS_BEDROCK/deployments/devnetL1/Vmeta3Portal.json)
...
jq ". | .alloc.\"4200000000000000000000000000000000000015\".code = \"$L1_BLOCK_INFO_BYTECODE\"" < ./ops-bedrock/genesis-l2.json | \
  ...
  jq ". | .alloc.\"4200000000000000000000000000000000000012\".code = \"$VMETA3_MINTABLE_TOKEN_FACTORY_BYTECODE\"" | \
